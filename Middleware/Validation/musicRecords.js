...
const allArtistsFromGenreValidation = async (req, res, next) => {
  try {
    const schema = Joi.object().keys({
      genreName: Joi.string().error(new CustomError(406, '00003', 'Invalid genre format')).required(),
    }).required()
    const requestData = req.query
    if (schema.validate(requestData).error) {
      const validationError = schema.validate(requestData).error
      logger.info(`${req.loggerID} - ${406} - ${validationError.errorMessage} - ${validationError.messageID}`)
    ...
    logger.error(`${req.loggerID} - ${err.code ? err.code : 500} - ${err.errorMessage ? err.errorMessage : 'Internal server error'} - ${err.messageID ? err.messageID : '00000'}`)
...
const allAlbumsFromArtistValidation = async (req, res, next) => {
  try {
    const schema = Joi.object().keys({
      artistName: Joi.string().error(new CustomError(406, '00005', 'Invalid artist name format')).required(),
    }).required()
    const requestData = req.query
    if (schema.validate(requestData).error) {
      const validationError = schema.validate(requestData).error
